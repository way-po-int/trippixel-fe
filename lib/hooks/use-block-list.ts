...
export const blockListQueryKey = (planId: string, params: GetBlockListParams) =>
  ["block-list", planId, params.day, params.page ?? 0, params.size ?? 20] as const;
...
export const useBlockList = (options: UseBlockListOptions) => {
  ...
  return useQuery<BlockListResponse, AxiosError>({
    queryKey: blockListQueryKey(planId ?? "", params ?? { day: 0, page: 0, size: 20 }),
...
export const useSetBlockListCache = () => {
  ...
  return (planId: string, params: GetBlockListParams, next: BlockListResponse) => {
    queryClient.setQueryData<BlockListResponse>(blockListQueryKey(planId, params), next);
  };
};
